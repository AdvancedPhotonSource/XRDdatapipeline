...
class Settings:
    ...
    colors: dict = field(default_factory= lambda: {
        ...
        "minus_spot_line": ColorSettings("Base \u2013 Spot Masked Integral Line","black"),
        "minus_arcs_line": ColorSettings("Base \u2013 Texture Masked Integral Line","black"),
...
class SettingsWindow(QtWidgets.QWidget):
    ...
    def browse_imctrl(self):
        imctrl_file_name = QtWidgets.QFileDialog.getOpenFileName(None,"Choose Configuration File",self.directory_text.text(),"Imctrl and PONI files (*.imctrl *.poni)")
...
class KeyPressWindow(QtWidgets.QWidget):
    def __init__(self,directory=".",imagecontrol=""):
        ...
        # }
        self.settings = Settings(directory,imagecontrol,(2880,2880),0,0,[],{},0,0)
        ...
        self.filemenu.addAction("Load Directory and Image Control File",self.choose_dir)
        ...
        self.cursor_label = QtWidgets.QLabel("<span style='font-size: 12pt'>x=%0.0f,   y=%0.0f</span>,   <span style='color: red; font-size: 12pt'>2Theta=%0.1f,    Azim=%0.1f,</span>    <span style='font-size: 12pt'>Q=%0.0f,    d=%0.0f</span>" % (0,0,0,0,0,0))
        self.cursor_label.setMinimumSize(500,50)
        self.integral_cursor_label = pg.TextItem(text="tth= \nq= \nd= ",anchor=(0,1))
        self.integral_widget.integral_view.addItem(self.integral_cursor_label, ignoreBounds=True)
        self.imageview.view.scene().sigMouseMoved.connect(self.mouseMovedImage)
        self.integral_widget.integral_view.scene().sigMouseMoved.connect(self.mouseMovedIntegral)
        #self.integral_widget.integral_view.scene().sigMouseHover.connect(self.mouseHoverIntegral)
        self.tabbed_area.contour_widget.view.scene().sigMouseMoved.connect(self.mouseMovedContour)
        #self.tabbed_area.stats_widget.stats_view.scene().sigMouseMoved.connect(self.mouseMovedStats)
        self.tabbed_area.contour_widget.view.scene().sigMouseClicked.connect(self.mouseClickedContourChangeImage)
        ...
        self.linked_axes_checkbox.stateChanged.connect(self.linked_axes_checkbox_changed)

        self.live_view_image_checkbox = QtWidgets.QCheckBox("Live update")
        self.live_view_image_checkbox.stateChanged.connect(self.live_view_image_checkbox_changed)
        ...
        self.layout.addWidget(self.integral_widget.texturemasked_integral_checkbox,3,8)
    ...
    def update_dir(self):
        ...
        self.name_label.setText("<span style='font-size: 12pt'>{0}</span>".format(self.settings.tiflist[self.settings.keylist[self.settings.curr_key]][self.settings.curr_pos]))

    def update_tiflist(self):
        # global tiflist, keylist, curr_key, curr_pos
        fulltiflist=sorted(glob.glob(self.settings.directory+'/*.tif'), key=os.path.getctime)
        keylist = []
        tiflist = {}
        ...
        self.settings.image_size = self.get_image_size(self.settings.directory + "\\" + self.settings.tiflist[self.settings.keylist[self.settings.curr_key]][self.settings.curr_pos]+".tif")
    ...
    def check_for_updates(self):
        ...
        last_completed = last_completed.split("\\")[-1].split(self.tabbed_area.contour_widget.integral_extension)[0]
        #print(tiflist[keylist[curr_key]])
        completed_pos = self.settings.tiflist[self.settings.keylist[self.settings.curr_key]].index(last_completed)
        #print(completed_pos)
    ...
    def forward(self):
        ...
        self.settings.curr_pos = self.settings.curr_pos % len(self.settings.tiflist[self.settings.keylist[self.settings.curr_key]])
        self.updateImages()
    ...
    def mouseMovedImage(self,evt):
        ...
        if self.imageview.view.sceneBoundingRect().contains(pos):
            ...
            if x_val > 0 and x_val < self.imageview.image_data.shape[1] and y_val > 0 and y_val < self.imageview.image_data.shape[0]:
                ...
                self.cursor_label.setText("<span style='font-size: 12pt'>x=%0.0f,   y=%0.0f,    z=%0.0f</span>,   <span style='color: red; font-size: 12pt'>2Theta=%0.2f,    Azim=%0.1f,</span>    <span style='font-size: 12pt'>Q=%0.2f,    d=%0.2f</span>" % (mousePoint.x(), mousePoint.y(), z, tth, azim, Q, d))
                if self.vLineCheckbox.isChecked():
                    integral_point = mousePoint
                    integral_point.setY(self.integral_widget.integral_view.getAxis("left").range[0])
    ...
    def mouseMovedIntegral(self,evt):
        ...
        if self.integral_widget.integral_view.sceneBoundingRect().contains(pos):
            ...
            if self.vLineCheckbox.isChecked():
                ...
                d = tth_to_d(tth,self.settings.wavelength)
                self.integral_cursor_label.setText("tth={0:0.2f}\nQ={1:0.2f}\nd={2:0.2f}".format(tth,Q,d))
            if self.circleCheckbox.isChecked():
                if self.x_axis_choice.currentIndex() == 0:
                    self.imageview.update_tth_circle(mousePoint.x())
                elif self.x_axis_choice.currentIndex() == 1:
                    self.imageview.update_tth_circle(q_to_tth(mousePoint.x(),self.settings.wavelength))
            if self.tabbed_area.contour_widget.tth_line_checkbox.isChecked():
    ...
    def circleCheckbox_changed(self,evt):
        if not self.circleCheckbox.isChecked():
            ...
            self.imageview.tth_circle.updateImage(self.imageview.tth_circle_data.full_data)
        else:
    ...
    def linked_axes_checkbox_changed(self,evt):
        if self.linked_axes_checkbox.isChecked():
            self.tabbed_area.contour_widget.view.setXLink(self.integral_widget.integral_view.getViewBox())
        else:
    ...
    def pause(self):
        if self.live_view_image_checkbox.isChecked():
            self.live_view_image_checkbox.stateChanged.disconnect(self.live_view_image_checkbox_changed)
...
class MainImageView(pg.GraphicsLayoutWidget):
    def __init__(self,settings: Settings,parent=None):
        ...
        self.predef_mask_data = image_mask(self.settings.image_size,self.settings.colors["predef_mask"].color)
        self.nonpositive_mask_data = image_mask(self.settings.image_size,self.settings.colors["nonpositive_mask"].color)
        ...
        self.outlier_mask_data = image_mask(self.settings.image_size,self.settings.colors["outlier_mask"].color) # green
        ...
        self.spot_mask_data = image_mask(self.settings.image_size,self.settings.colors["spot_mask"].color) # darkcyan
        self.arcs_mask_data = image_mask(self.settings.image_size,self.settings.colors["arcs_mask"].color) # maroon
        ...
        self.masks = {self.predef_mask: [self.predef_mask_data,"_predef.tif"],
        ...
        self.tth_circle_data = image_mask(self.settings.image_size,self.settings.colors["tth_circle_mask"].color)
        self.tth_circle_data.set_opacity(.5)
    ...
    def update_dir(self):
        ...
        self.nonpositive_mask_data.set_color(self.settings.colors["nonpositive_mask"].color)
    ...
    def update_image_data(self, xy_reset=False, z_reset=False):
        # global tiflist, keylist, curr_key, curr_pos
        self.image_data = tf.imread(self.settings.directory + "\\" + self.settings.tiflist[self.settings.keylist[self.settings.curr_key]][self.settings.curr_pos] + ".tif")
        if z_reset:
            maxval = np.percentile(self.image_data,99.9)
            self.image.updateImage(self.image_data, autoRange=xy_reset, autoLevels=False)
            self.intensityBar.setLevels(min=0.0,max=maxval)
        else:
            self.image.updateImage(self.image_data, autoRange=xy_reset, autoLevels=z_reset, autoHistogramRange=z_reset)
    ...
    def update_masks_data(self):
        ...
        for mask,vals in self.masks.items():
            ...
            file_name = self.settings.directory + "\\masks\\" + self.settings.tiflist[self.settings.keylist[self.settings.curr_key]][self.settings.curr_pos] + vals[1]
...
class IntegralView(pg.GraphicsLayoutWidget):
    def __init__(self,parent,settings: Settings):
        ...
        self.base_integral = self.integral_view.plot(pen=self.settings.colors["base_line"].color)
        self.masked_integral = self.integral_view.plot(pen=self.settings.colors["outlier_line"].color)
        self.spotmasked_integral = self.integral_view.plot(pen=self.settings.colors["spot_line"].color)
        self.texturemasked_integral = self.integral_view.plot(pen=self.settings.colors["arcs_line"].color)
        self.spots_diff_integral = self.integral_view.plot(pen=self.settings.colors["minus_spot_line"].color)
        self.arcs_diff_integral = self.integral_view.plot(pen=self.settings.colors["minus_arcs_line"].color)
        ...
        self.base_integral_checkbox.stateChanged.connect(self.base_integral_checkbox_changed)

        self.masked_integral_checkbox = QtWidgets.QCheckBox("Outlier Masked")
        self.masked_integral_checkbox.setChecked(True)
        self.masked_integral_checkbox.stateChanged.connect(self.masked_integral_checkbox_changed)

        self.spotmasked_integral_checkbox = QtWidgets.QCheckBox("Spot Masked")
        self.spotmasked_integral_checkbox.setChecked(True)
        self.spotmasked_integral_checkbox.stateChanged.connect(self.spotmasked_integral_checkbox_changed)
        ...
        self.texturemasked_integral_checkbox.stateChanged.connect(self.texturemasked_integral_checkbox_changed)

        self.spots_diff_integral_checkbox = QtWidgets.QCheckBox("Base \u2013 Spot Masked")
        self.spots_diff_integral_checkbox.stateChanged.connect(self.spots_diff_integral_checkbox_changed)
        self.arcs_diff_integral_checkbox = QtWidgets.QCheckBox("Base \u2013 Texture Masked")
        self.arcs_diff_integral_checkbox.stateChanged.connect(self.arcs_diff_integral_checkbox_changed)
    ...
    def update_integral_data(self):
        ...
        # global tiflist, keylist, curr_key, curr_pos
        integral_infile_piece = self.settings.directory + "\\integrals\\" + self.settings.tiflist[self.settings.keylist[self.settings.curr_key]][self.settings.curr_pos]
        integrals_dict = {"_base.xye":self.integral_data,
        ...
        for ext,vals in integrals_dict.items():
            ...
            try:
                ...
                print("Exception reading in integral data for {0}. Setting to 0.".format(integral_infile_piece+ext))
                vals[:,:] = 0
            if new_vals.shape != vals.shape:
                print("Shape of incoming data {0} not equal to config shape {1} for integral {2}. Setting values to 0. If the config shape looks incorrect, go to Settings->outChannels and set it to {3}.".format(new_vals.shape,vals.shape,integral_infile_piece+ext,new_vals.shape[0]))
                vals[:,:] = 0
            try:
                vals[:,:] = new_vals
            except:
                print("Exception setting integral data for {0}. Setting to 0.".format(integral_infile_piece,ext))

        if self.axis_type == 0:
            self.base_integral.setData(self.integral_data[:,0],self.integral_data[:,1])
            self.masked_integral.setData(self.masked_integral_data[:,0],self.masked_integral_data[:,1] + self.integral_offset.value())
            self.spotmasked_integral.setData(self.spotmasked_integral_data[:,0],self.spotmasked_integral_data[:,1] + 2*self.integral_offset.value())
            self.texturemasked_integral.setData(self.texturemasked_integral_data[:,0],self.texturemasked_integral_data[:,1] + 3*self.integral_offset.value())
            ...
            self.base_integral.setData(tth_to_q(self.integral_data[:,0],self.settings.wavelength),self.integral_data[:,1])
            self.masked_integral.setData(tth_to_q(self.masked_integral_data[:,0],self.settings.wavelength),self.masked_integral_data[:,1] + self.integral_offset.value())
            self.spotmasked_integral.setData(tth_to_q(self.spotmasked_integral_data[:,0],self.settings.wavelength),self.spotmasked_integral_data[:,1] + 2*self.integral_offset.value())
            self.texturemasked_integral.setData(tth_to_q(self.texturemasked_integral_data[:,0],self.settings.wavelength),self.texturemasked_integral_data[:,1] + 3*self.integral_offset.value())
            self.spots_diff_integral.setData(tth_to_q(self.spotmasked_integral_data[:,0],self.settings.wavelength),self.integral_data[:,1]-self.spotmasked_integral_data[:,1])
            self.arcs_diff_integral.setData(tth_to_q(self.texturemasked_integral_data[:,0],self.settings.wavelength),self.integral_data[:,1]-self.texturemasked_integral_data[:,1])
    ...
    def sqrt_toggle(self,evt):
        if self.sqrt_checkbox.isChecked():
            ...
            self.integral_offset.setValue(int(np.round(np.sqrt(self.integral_offset.value()),decimals=-1)))
            # self.integral_offset.valueChanged.connect(self.update_integral_offset)
            self.integral_offset.setSingleStep(10)
            # Toggle data. Diff integrals have some negative values.
            self.base_integral.setData(self.integral_data[:,0],np.sqrt(self.integral_data[:,1]))
            self.masked_integral.setData(self.masked_integral_data[:,0],np.sqrt(self.masked_integral_data[:,1]) + self.integral_offset.value())
            self.spotmasked_integral.setData(self.spotmasked_integral_data[:,0],np.sqrt(self.spotmasked_integral_data[:,1]) + 2*self.integral_offset.value())
            self.texturemasked_integral.setData(self.texturemasked_integral_data[:,0],np.sqrt(self.texturemasked_integral_data[:,1]) + 3*self.integral_offset.value())
            ...
            self.integral_offset.setValue(int(np.round(self.integral_offset.value()**2,decimals=-2)))
            self.integral_offset.setSingleStep(100)
            # Toggle data. Diff integrals have some negative values.
            self.base_integral.setData(self.integral_data[:,0],self.integral_data[:,1])
            self.masked_integral.setData(self.masked_integral_data[:,0],self.masked_integral_data[:,1] + self.integral_offset.value())
...
class StatsView(pg.GraphicsLayoutWidget):
    def __init__(self,parent,settings: Settings):
        ...
        self.spots_histogram_area_curve = pg.PlotCurveItem(fillLevel=0,brush=(0,0,255,80))
        self.spots_histogram_Q_curve = pg.PlotCurveItem(fillLevel=0,brush=(0,255,0,80))
        ...
        self.histogram_type_dict = {"Spot Area":self.spots_histogram_area_curve,
        ...
        self.histogram_type_select.currentIndexChanged.connect(self.histogram_type_changed)
    ...
    def histogram_type_changed(self,evt):
        ...
        if evt != 2:
            self.legend.addItem(self.histogram_type_dict[self.histogram_types[evt]],self.histogram_types[evt])

    def update_stats_data(self):
        stats_infile = self.settings.directory + "\\stats\\" + self.settings.tiflist[self.settings.keylist[self.settings.curr_key]][self.settings.curr_pos] + "_spots_hist.npy"
        ...
        self.spots_histogram_area_curve.setData(self.area_bins,self.spots_histogram_area,stepMode="center")
        self.spots_histogram_Q_curve.setData(self.Q_bins,self.spots_histogram_Q,stepMode="center")
...
class CSimView(pg.GraphicsLayoutWidget):
    def __init__(self, parent, settings: Settings):
        ...
        self.methods = {"Cosine Similarity": "_csim.txt",
    ...
    def update_data(self):
        ...
        for k,v in self.methods.items():
            ...
            filename0 = self.settings.directory + "\\stats\\" + self.settings.keylist[self.settings.curr_key] + "00000" + v
            filename  = self.settings.directory + "\\stats\\" + self.settings.keylist[self.settings.curr_key][:-1] + v # testing shows the - at the end, need to remove
...
class ContourView(pg.GraphicsLayoutWidget):
    def __init__(self, parent, settings: Settings):
        ...
        self.live_update_checkbox.stateChanged.connect(self.live_update_checkbox_changed)
        ...
        self.integral_type_dict = {"Base":"_base.xye",
    ...
    def update_integral_list(self):
        # global keylist, curr_key
        self.integral_filelist = sorted(glob.glob(self.settings.directory + "\\integrals\\" + self.settings.keylist[self.settings.curr_key] + "*" + self.integral_extension), key=os.path.getctime)
        ...
        if (len(self.integral_filelist) > 0) and (time.time() - os.path.getctime(self.integral_filelist[-1]) < 0.5):
    ...
    def auto_set_spacing(self):
        ...
        while len(self.integral_filelist[self.live_min::self._temp_auto_spacing]) >= (self.live_max_lines_visible + 1):
    ...
    def append_integral_data(self, max=None):
        ...
        if max == None:
            ...
            file_subset = self.integral_filelist[self.live_min:max:self.live_spacing]
        ...
        if self.x_axis_type == 0:
            self.contour_image.setRect(self.tthvals[0],self.yvals[0],self.tthvals[-1]-self.tthvals[0],self.yvals[-1]+self.live_spacing-self.yvals[0])
        elif self.x_axis_type == 1:
            self.contour_image.setRect(self.qvals[0],self.yvals[0],self.qvals[-1]-self.qvals[0],self.yvals[-1]+self.live_spacing-self.yvals[0])
    ...
    def update_integral_data(self):
        ...
        if self._temp_auto_spacing == self.live_spacing:
            ...
            if self._temp_auto_spacing % self.live_spacing == 0:
                self.integral_data = self.integral_data[0::int(self._temp_auto_spacing/self.live_spacing)]
                self.yvals = self.yvals[0::int(self._temp_auto_spacing/self.live_spacing)]
                self.live_spacing = self._temp_auto_spacing
                self.live_integral_step.valueChanged.disconnect(self.live_integral_step_changed)
...
class TabbedArea(QtWidgets.QTabWidget):
    def __init__(self,parent,settings: Settings):
        ...
        self.contour_layout.addWidget(self.contour_widget.live_integral_step_label,6,4)
