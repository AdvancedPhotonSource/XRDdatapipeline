...
class ImageAndMask(QtWidgets.QWidget):
    def __init__(self,cmap,is_first_image=False):
        ...
        # link to first image
        self.link_to_first_image_checkbox = QtWidgets.QCheckBox("Link intensity to first image")
        # default to (0,99.9th)
        ...
        self.image_scaling_checkbox.stateChanged.connect(self.image_scaling_checkbox_changed)
        self.gradient_scaling_checkbox.stateChanged.connect(self.gradient_scaling_checkbox_changed)
...
class KeyPressWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.directory_widget = FileSelect("Directory:",default_text=".",isdir=True,)
        self.directory = "."
        ...
        self.image_1.imview.getHistogramWidget().sigLevelChangeFinished.connect(self.update_other_histo_ranges)
        self.image_2.link_to_first_image_checkbox.stateChanged.connect(self.zlink_2_to_1)
        self.image_3.link_to_first_image_checkbox.stateChanged.connect(self.zlink_3_to_1)
        self.image_4.link_to_first_image_checkbox.stateChanged.connect(self.zlink_4_to_1)
    ...
    def update_tiflist(self,reset=False):
        ...
        for tif in self.fulltiflist:
            short = tif.split("\\")[-1]
            key = short.split("-")[0] # grab label at start of file name, eg "Sam4". Need to work on this, as some are things like "Dewen-4"
            if key not in self.keylist:
                self.keylist.append(key)
                self.tiflist[key] = []
            initialimage = short[0:re.search(r'(\d+)\D+$', short).end(1)] # string from the start to the end of the last set of numbers.
    ...
    def updateImages(self,autoScaleRange=True,resetHistoRange=False):
        ...
        image_1_data = self.findImage(subdir=self.image_1.image_dir.text(),ext=self.image_1.image_ext.text())
        image_2_data = self.findImage(subdir=self.image_2.image_dir.text(),ext=self.image_2.image_ext.text())
        image_3_data = self.findImage(subdir=self.image_3.image_dir.text(),ext=self.image_3.image_ext.text())
        image_4_data = self.findImage(subdir=self.image_4.image_dir.text(),ext=self.image_4.image_ext.text())
        self.image_1.imview.setImage(image_1_data,autoRange=autoScaleRange,autoLevels=autoScaleRange,autoHistogramRange=autoScaleRange)
        self.image_2.imview.setImage(image_2_data,autoRange=autoScaleRange,autoLevels=autoScaleRange,autoHistogramRange=autoScaleRange)
        self.image_3.imview.setImage(image_3_data,autoRange=autoScaleRange,autoLevels=autoScaleRange,autoHistogramRange=autoScaleRange)
        self.image_4.imview.setImage(image_4_data,autoRange=autoScaleRange,autoLevels=autoScaleRange,autoHistogramRange=autoScaleRange)

        self.addMaskImage(ext=self.image_1.mask_ext.text(),image_item=self.image_1.immask)
        self.addMaskImage(ext=self.image_2.mask_ext.text(),image_item=self.image_2.immask)
        self.addMaskImage(ext=self.image_3.mask_ext.text(),image_item=self.image_3.immask)
        self.addMaskImage(ext=self.image_4.mask_ext.text(),image_item=self.image_4.immask)
    ...
    def findImage(self,subdir=".",ext="",except_shape=(2880,2880)):
        try:
            im = imread(self.directory+"\\"+subdir+"\\"+self.tiflist[self.keylist[self.curr_key]][self.curr_pos]+ext+".tif")
        except:
    ...
    def forward(self):
        ...
        self.image_name.setText(self.tiflist[self.keylist[self.curr_key]][self.curr_pos])
    ...
    def update_other_histo_ranges(self,histo):
        ...
        if self.image_2.link_to_first_image_checkbox.isChecked():
            self.image_2.imview.getHistogramWidget().setLevels(min=levels[0],max=levels[1])
        if self.image_3.link_to_first_image_checkbox.isChecked():
            self.image_3.imview.getHistogramWidget().setLevels(min=levels[0],max=levels[1])
        if self.image_4.link_to_first_image_checkbox.isChecked():
            self.image_4.imview.getHistogramWidget().setLevels(min=levels[0],max=levels[1])

    def zlink_2_to_1(self,state):
        if state:
            self.image_2.imview.getHistogramWidget().vb.setYLink(self.image_1.imview.getHistogramWidget().vb)
        else:
            self.image_2.imview.getHistogramWidget().vb.setYLink(None)
    def zlink_3_to_1(self,state):
        if state:
            self.image_3.imview.getHistogramWidget().vb.setYLink(self.image_1.imview.getHistogramWidget().vb)
        else:
            self.image_3.imview.getHistogramWidget().vb.setYLink(None)
    def zlink_4_to_1(self,state):
        if state:
            self.image_4.imview.getHistogramWidget().vb.setYLink(self.image_1.imview.getHistogramWidget().vb)
        else:
