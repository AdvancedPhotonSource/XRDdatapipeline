...
# TODO: swap to generic location. Also note at least polymask is version-dependent.
sys.path.insert(0, 'C:/Users/amerritt/Documents/project/bindist')
...
class image_monitor(RegexMatchingEventHandler):
    ...
    def on_created(self,event):
        ...
        print("Directory: {0}, Name: {1}, Number: {2}, Extension: {3}".format(results[0].group("directory"),results[0].group("name"),results[0].group("number"),results[0].group("ext")))
        #number -> actual int
        ...
        # self.queue.append([event.src_path[:-9],results[0].group("name"),results[0].group("number")])
        self.queue.append([event.src_path,results[0].group("name"),results[0].group("number"),results[0].group("ext")])
...
class file_select(QtWidgets.QWidget):
    ...
    def select_file(self):
        if self.isdir:
            location = QtWidgets.QFileDialog.getExistingDirectory(None,"Select Directory")
            #self.file_select_button.setText(location)
            self.file_name.setText(location)
        else:
            location = QtWidgets.QFileDialog.getOpenFileName(None,"Select File",self.startdir,self.ext)
...
def MakeMaskMap(data,masks,iLim,jLim,tamp):
    ...
    if frame:
        tam = np.ma.mask_or(tam,np.ma.make_mask(pm.polymask(nI*nJ,tax,
    ...
    for X,Y,rsq in masks['Points'].T:
        tam = np.ma.mask_or(tam,np.ma.getmask(np.ma.masked_less((tax-X)**2+(tay-Y)**2,rsq)))
    if tam.shape: 
...
def MakeUseMask(data,masks,blkSize=128):
    ...
    useMask = []
    tamp = np.ma.make_mask_none((1024*1024))       #NB: this array size used in the fortran polymask module
...
def MakeUseTA(data,blkSize=128):
    ...
    for iBlk in range(nYBlks):
        ...
        for jBlk in range(nXBlks):
            ...
            TA = np.dstack((np.ma.getdata(TA[1]),np.ma.getdata(TA[0]),np.ma.getdata(TA[2]),np.ma.getdata(TA[3])))    #azimuth, 2-theta, dist, pol
...
def MakeFrameMask(data,frame):
    ...
    for iBlk in range(nXBlks):
        ...
        for jBlk in range(nYBlks):
            ...
            tamp = np.ma.make_mask(pm.polymask(nI*nJ,tax.flatten(),
                tay.flatten(),len(frame),frame,tamp)[:nI*nJ])^True  #switch to exclude around frame
            if tamp.shape:
                tamp = np.reshape(tamp[:nI*nJ],(nI,nJ))
                tam[iBeg:iFin,jBeg:jFin] = np.ma.mask_or(tamp[0:nI,0:nJ],tam[iBeg:iFin,jBeg:jFin])
            else:
...
def pol_correct(image,polmap):
    ...
    image_p = np.array(image_p/polmap,dtype=np.int32) #polmap showing high values in center column dropping to low values at right/left edges
    return image_p
...
#G2sc
def GeneratePixelMask(img,esdMul=3.0,ttmin=0.,ttmax=180.,
...
def GetDetectorXY2(dsp,azm,data):
    ...
    if radii[0] > 0.:
        ...
        Q = np.sqrt(2.)*radii[0]*radii[1]*np.sqrt(R-2.*zdis**2*npsind(azm-phi)**2)
        P = 2.*radii[0]**2*zdis*npcosd(azm-phi)
        radius = (P+Q)/R
        ...
        if tilt > 0.:
            ...
            offset = -f
            xy = [-R*npcosd(azm)-offset,R*npsind(azm)]
        xy = -np.array([xy[0]*npcosd(phi)+xy[1]*npsind(phi),xy[0]*npsind(phi)-xy[1]*npcosd(phi)])
        xy += cent
    if data['det2theta']:
...
def pytorch_integrate(image, mask, tth_idx, tth_val, raveled_pol, raveled_dist, tth_size):
    data = image.ravel()
    data = torch.from_numpy(data)
    mask = mask.ravel()
    mask = ~mask
    mask = torch.from_numpy(mask)
    data = data / raveled_pol * raveled_dist**1.5   # the dist map is squared distance, here it needs to be cubic
...
def gradient_cache(image_shape,center,footprint):
    ...
    rel_coords = np.indices(footprint.shape) - central_footprint_point[:,None,None]*np.ones_like(footprint)
    for i,j in np.ndindex(distances.shape):
        if footprint[i,j] != 0:
            distances[i,j] = np.sqrt((i-central_footprint_point[0])**2 + (j-central_footprint_point[1])**2)
            direction_vectors[:,i,j] = rel_coords[:,i,j] / np.linalg.norm(rel_coords[:,i,j],axis=0)
    ...
    nonzero_footprint_x = np.sum(np.ones_like(footprint)[np.nonzero(footprint * x_dots)])
    nonzero_footprint_y = np.sum(np.ones_like(footprint)[np.nonzero(footprint * y_dots)])
    ...
    for i in range(footprint.shape[0]):
        for j in range(footprint.shape[1]):
            if (i == central_footprint_point[0]) and (j == central_footprint_point[1]):
                kernel_x[i,j] = -(nonzero_footprint_x/sum_footprint_x)*np.sum(footprint * x_dots / np.where(distances == 0, 999, distances)) # distances should only be zero at center, where footprint = 0 anyway
                kernel_y[i,j] = -(nonzero_footprint_y/sum_footprint_y)*np.sum(footprint * y_dots / np.where(distances == 0, 999, distances))
            elif footprint[i,j] == 0:
                kernel_x[i,j] = 0
                kernel_y[i,j] = 0
            else:
                kernel_x[i,j] = (1/sum_footprint_x) * footprint[i,j] * x_dots[i,j] / distances[i,j]
                kernel_y[i,j] = (1/sum_footprint_x) * footprint[i,j] * y_dots[i,j] / distances[i,j]
    ...
    t1 = time.time()
    print("Gradient time spent on calculations that should be saved to cache: {0:.2f}s".format(t1-t0))
    return_dict = {'r_hat': r_hat, 'phi_hat': phi_hat, 'kernel_x': kernel_x, 'kernel_y': kernel_y}
    return return_dict

def radial_and_azim_gradient(image, r_hat, phi_hat, kernel_x, kernel_y, azim_only = False):
...
def qwidth_area_classification(om,Qmap,azmap,min_arc_area = 100, Q_max = 0.1, azim_min = 3.5, compare_shape = True, area_Q_shape_min = 150000, azim_Q_shape_min = 100, return_time = False):
    # azim_Q: 150
    # area_Q: 4500
    # area_Q^2: 350000
    if return_time: time0 = time.time()
    om = np.array(om)
    ...
    props_table = ski.measure.regionprops_table(labeled_mask,intensity_image=Qmap,properties=props)
    props_table = pd.DataFrame(props_table)
    props_table['diffs_Q'] = props_table['intensity_max'] - props_table['intensity_min']
    props_table_azim = ski.measure.regionprops_table(labeled_mask,intensity_image=azmap,properties=['label','intensity_min','intensity_max'])
    props_table_azim = pd.DataFrame(props_table_azim)
    props_table['diffs_azim'] = props_table_azim['intensity_max'] - props_table_azim['intensity_min'] # same label should mean same order, same index
    ...
    props_table['area_over_width2'] = props_table['area'] / (props_table['diffs_Q']**2)
    ...
    arcs_bool = np.logical_and(props_table.loc[:,'diffs_azim'].values > azim_min, arcs_bool)
    if compare_shape:
        ...
        arcs_bool = np.logical_and(props_table.loc[:,'area_over_width2'].values > area_Q_shape_min, arcs_bool)
...
def split_grad_with_Q(image,om,arc_mask,gradient_dict,qmap,azmap,threshold_percentile=0.1,return_grad=False,return_partials=False,interpolate=False,predef_mask=None,predef_mask_extended=None):
    if interpolate:
        if predef_mask is None:
            ...
            radial_grad, azim_grad = radial_and_azim_gradient(interpolated_image,gradient_dict['r_hat'],gradient_dict['phi_hat'],gradient_dict['kernel_x'],gradient_dict['kernel_y'])
            azim_grad_2 = radial_and_azim_gradient(azim_grad,gradient_dict['r_hat'],gradient_dict['phi_hat'],gradient_dict['kernel_x'],gradient_dict['kernel_y'],azim_only=True)
            radial_grad_2,_ = radial_and_azim_gradient(radial_grad,gradient_dict['r_hat'],gradient_dict['phi_hat'],gradient_dict['kernel_x'],gradient_dict['kernel_y'])
    else:
        radial_grad, azim_grad = radial_and_azim_gradient(image,gradient_dict['r_hat'],gradient_dict['phi_hat'],gradient_dict['kernel_x'],gradient_dict['kernel_y'])
        azim_grad_2 = radial_and_azim_gradient(azim_grad,gradient_dict['r_hat'],gradient_dict['phi_hat'],gradient_dict['kernel_x'],gradient_dict['kernel_y'],azim_only=True)
        radial_grad_2,_ = radial_and_azim_gradient(radial_grad,gradient_dict['r_hat'],gradient_dict['phi_hat'],gradient_dict['kernel_x'],gradient_dict['kernel_y'])
    ...
    props_table = ski.measure.regionprops_table(labeled_mask,properties=props,intensity_image=qmap) # replace with array of label values
    ...
    props_table['Q_median'] = [np.median(qmap[labeled_mask == label]) for label in props_table['label']]
    ...
    props_table['central_slice'] = [qmap[labeled_mask == label] - center < 0.02 for label,center in zip(props_table['label'],props_table['Q_median'])]
    ...
    props_table['second_radial'] = [radial_grad_2[labeled_mask == label][central_slice] for label, central_slice in zip(props_table['label'],props_table['central_slice'])]
    # print(props_table[['coords','second_radial']])
    # print(props_table[['Q_median','central_slice','second_radial']])
    props_table['radial_grad_percentile'] = [np.percentile(radial_grads,20) for radial_grads in props_table['second_radial']]
    ...
    # small_cluster_indices = props_table.iloc[small_clusters].index
    # props_table.drop(small_cluster_indices,axis=0,inplace=True)
    props_table_shortened = props_table.drop(props_table.iloc[~props_table['on_arc'].values].index,axis=0)
    # print(props_table_shortened[['second_radial','radial_grad_percentile','on_arc']])
    arc_cluster_indices = props_table_shortened['label'].values # using index is one off
    ...
    if np.max(gradient_mask) > 0:
        ...
        gradient_props_table = ski.measure.regionprops_table(labeled_gradients,properties=gradient_props)
        ...
        gradient_props_table['cluster_label'] = [labeled_mask[int(centroid_0),int(centroid_1)] for centroid_0, centroid_1 in zip(gradient_props_table['centroid-0'],gradient_props_table['centroid-1'])]
        gradient_props_table['azim_centroid'] = [azmap[int(centroid_0),int(centroid_1)] for centroid_0, centroid_1 in zip(gradient_props_table['centroid-0'],gradient_props_table['centroid-1'])]
        ...
        for i in range(len(labels)):
            ...
            if labels[i] == 0:
                ...
                diff = props_table.loc[props_table['label']==labels[i],'diffs'].values[0]
            vals.append(diff)
        vals = np.array(vals)
        ...
        gradient_props_table['cuts'] = [np.logical_and(labeled_mask == label, modulo_range(azmap,center_azim,azim_width)) for label, center_azim, azim_width in zip(gradient_props_table['cluster_label'],gradient_props_table['azim_centroid'],gradient_props_table['azim_width'])]
        if predef_mask_extended is not None:
            gradient_props_table['labeled_cuts'] = [ski.measure.label(cuts) for cuts in gradient_props_table['cuts']]
            ...
            gradient_props_table['new_cuts'] = [remove_overlaps(labeled_cuts,predef_mask_extended) for labeled_cuts in gradient_props_table['labeled_cuts']]
    ...
    spots_table = ski.measure.regionprops_table(labeled_spots,qmap,properties=spots_props)
...
def current_splitting_method(image,om,qmap,azmap,gradient_dict,threshold_percentile=0.1,return_steps = False, interpolate=False, predef_mask=None, predef_mask_extended=None):
    # t0 = time.time()
    ...
    base_spot, base_arc = qwidth_area_classification(om, qmap, azmap, min_arc_area=100, Q_max=0.08, azim_min=3.5, compare_shape=True)
    # t1 = time.time()
    ...
    if return_steps:
        spot_mask, arc_mask, spots_table, radial_grad_2, azim_grad_2, qgrad_arc_mask = split_grad_with_Q(image,om,base_arc,gradient_dict,qmap,azmap,threshold_percentile=threshold_percentile,return_partials=True,return_grad=True, interpolate=interpolate, predef_mask=predef_mask, predef_mask_extended=predef_mask_extended)
    else:
        spot_mask, arc_mask, spots_table = split_grad_with_Q(image,om,base_arc,gradient_dict,qmap,azmap,threshold_percentile=threshold_percentile, interpolate=interpolate, predef_mask=predef_mask, predef_mask_extended=predef_mask_extended)
...
class CacheCreator(QtCore.QObject):
    ...
    def __init__(self,cache,directory,filename,imctrlname,flatfield,imgmaskname,blkSize,logging=False):
        super().__init__()
        self.cache = cache
    ...
    def run(self):
        ...
        if self.logging: print("Creating cache")
        ...
        imsave.save(self.directory + "\\maps\\" + self.imctrlname.split("\\")[-1].split('/')[-1].split('.')[0] + "_predef.tif")
        imsave = Image.fromarray(flatfield_image)
        imsave.save(self.directory + "\\maps\\" + self.imctrlname.split("\\")[-1].split('/')[-1].split('.')[0] + "_flatfield.tif")
        ...
        getmaps(self.cache,self.imctrlname,self.directory+"\\maps")
        ...
        # pytorch integration
        self.cache['tth_idx'],self.cache['tth_val'],self.cache['raveled_pol'],self.cache['raveled_dist'],self.cache['tth_size'] = prepare_qmaps(self.cache['pixelTAmap'],self.cache['polscalemap'],self.cache['pixelsampledistmap'],self.cache['Image Controls']['IOtth'][0],self.cache['Image Controls']['IOtth'][1],self.cache['Image Controls']['outChannels'])
        ...
        # gradient info
        self.cache['gradient'] = gradient_cache(predef_mask['image'].shape,center,np.ones((3,3),dtype=np.uint))
...
class SingleIterator(QtCore.QObject):
    ...
    def __init__(self,cache,filename,imctrlname,imgmaskname,directory,name,number,ext,closing_method = "binary_closing",logging=False):
        super().__init__()
    ...
    def run(self):
        ...
        if self.logging: print("Pulling from cache")
        ...
        image_dict['image'] = flatfield_correct(image_dict['image'],self.cache['flatfield'])
        ...
        if self.logging: print("Adding in frame and nonzero mask to predefined mask")
        ...
        imsave.save(self.directory + '\\masks\\' + self.name + '-' + self.number + '_nonpositive.tif')
        ...
        predef_and_nonpositive = np.logical_or(nonpositive_mask,self.cache['predef_mask']['image'])
        predef_mask_extended = ski.morphology.binary_dilation(predef_and_nonpositive,footprint=ski.morphology.square(7)) # extend out by three pixels; use for determining whether something is nearby
        # print(mask_nonzero.dtype)
        # print(np.array(self.cache['predef_mask']['image']).dtype)
        # print(predef_mask)
        frame_and_predef = np.logical_or(predef_and_nonpositive,self.cache['FrameMask'])
        ...
        if self.logging: print("Generating pixel mask")
        ...
        GeneratePixelMask(image_dict,esdMul=self.cache['esdMul'],FrameMask=frame_and_predef,ThetaMap=self.cache['maskTmap'])
        ...
        imsave.save(self.directory + '\\masks\\' + self.name + '-' + self.number + '_om.tif')
        single_iter_times.append(time.time())
        time_checkpoints.append('Outlier Mask')
        #close holes
        if self.logging: print("Closing the mask")
        if self.closing_method == "binary_closing":
            closed_mask = ski.morphology.binary_closing(outlier_mask,footprint=ski.morphology.square(3))
            imsave = Image.fromarray(closed_mask)
            imsave.save(self.directory + '\\masks\\' + self.name + '-' + self.number + '_closedmask.tif')
        ...
        # split mask
        if self.logging: print("Splitting the mask")
        ...
        split_spots, split_arcs, spots_table, base_arc, qgrad_arc, azim_grad_2, radial_grad_2 = current_splitting_method(image_dict['image'],closed_mask,self.cache['pixelQmap'],self.cache['pixelAzmap'],self.cache['gradient'],return_steps=True,interpolate=False,predef_mask=nonpositive_mask,predef_mask_extended=predef_mask_extended)
        exp = ""
        imsave = Image.fromarray(split_spots)
        imsave.save(self.directory + '\\masks\\' + self.name + '-' + self.number + '_spots'+exp+'.tif')
        imsave = Image.fromarray(split_arcs)
        imsave.save(self.directory + '\\masks\\' + self.name + '-' + self.number + '_arcs'+exp+'.tif')
        imsave = Image.fromarray(base_arc)
        imsave.save(self.directory + '\\masks\\' + self.name + '-' + self.number + '_qwidth_arc'+exp+'.tif')
        imsave = Image.fromarray(qgrad_arc)
        imsave.save(self.directory + '\\masks\\' + self.name + '-' + self.number + '_qgrad_arc.tif')
        imsave = Image.fromarray(azim_grad_2)
        imsave.save(self.directory + '\\grads\\' + self.name + '-' + self.number + '_azim_grad_2.tif')
        imsave = Image.fromarray(radial_grad_2)
        imsave.save(self.directory + '\\grads\\' + self.name + '-' + self.number + '_radial_grad_2.tif')
        single_iter_times.append(time.time())
        ...
        if self.logging: print("Integrating...")
        ...
        hist_base = pytorch_integrate(image_dict['image'],frame_and_predef,self.cache['tth_idx'],self.cache['tth_val'],self.cache['raveled_pol'],self.cache['raveled_dist'],self.cache['tth_size'])
        ...
        hist_closed = pytorch_integrate(image_dict['image'],np.logical_or(closed_mask,frame_and_predef),self.cache['tth_idx'],self.cache['tth_val'],self.cache['raveled_pol'],self.cache['raveled_dist'],self.cache['tth_size'])
        ...
        hist_closedspotsmasked = pytorch_integrate(image_dict['image'],np.logical_or(split_spots,frame_and_predef),self.cache['tth_idx'],self.cache['tth_val'],self.cache['raveled_pol'],self.cache['raveled_dist'],self.cache['tth_size'])
        ...
        hist_closedarcsmasked = pytorch_integrate(image_dict['image'],np.logical_or(split_arcs,frame_and_predef),self.cache['tth_idx'],self.cache['tth_val'],self.cache['raveled_pol'],self.cache['raveled_dist'],self.cache['tth_size'])
        if self.logging: print("Integration complete")
        single_iter_times.append(time.time())
        ...
        hist,x_edges,y_edges = np.histogram2d(spots_table["area"].values,spots_table["intensity_mean"].values,100)
        with open(stats_prefix + '-' + self.number + '_spots_hist.npy','wb') as outfile:
        ...
        try:
            previous_image = ski.io.imread(self.directory+ '\\' + self.name + '-' + prev_number + self.ext).astype(np.float32)
        except:
            previous_image = image_dict['image'].astype(np.float32)
        csim_f = 1 - spatial.distance.cosine(np.array(image_dict['image'],dtype=np.float32).ravel(),self.cache['First image'].ravel())
        csim_p = 1 - spatial.distance.cosine(np.array(image_dict['image'],dtype=np.float32).ravel(),previous_image.ravel())
        ...
        for i in range(len(single_iter_times)-1):
            print("{0}: {1:.2f}".format(time_checkpoints[i],single_iter_times[i+1]-single_iter_times[i]))
...
class main_window(QtWidgets.QWidget):
    ...
    def __init__(self, directory=None, imctrl=None, imgmask=None):
        ...
        self.directory_widget = file_select("Directory:",default_text=directory,isdir=True,)
        #self.config_text = QtWidgets.QPushButton("Config file:")
        #self.config_loc = QtWidgets.QLabel()
        self.config_widget = file_select("Config file:", default_text=imctrl, startdir=self.directory_widget.file_name.text(), ext="Imctrl and PONI files (*.imctrl *.poni)")
        #self.predef_mask_text = QtWidgets.QPushButton("Predefined Mask:")
        #self.predef_mask_loc = QtWidgets.QLabel()
        self.flatfield_widget = file_select("Flat-field file:",startdir=self.directory_widget.file_name.text())
        self.predef_mask_widget = file_select("Predefined Mask:",default_text=imgmask, startdir=self.directory_widget.file_name.text())
        ...
        self.process_existing_images_checkbox = QtWidgets.QCheckBox("Process existing images")
        ...
        self.queue_length_info = QtWidgets.QLabel("Queue is {0} items long".format(len(self.queue)))
            ...
            if self.queue:
                ...
                if self.has_made_cache:
                    ...
                    if (time.time() - os.path.getmtime(self.queue[0][0]) > 1):
                        ...
                        self.timer.stop()
                        self.iteration_thread = QtCore.QThread()
                        self.iteration_worker = SingleIterator(self.cache,filename,self.imgctrl,self.imgmask,self.directory,name,number,ext)
                        self.iteration_worker.moveToThread(self.iteration_thread)
                        self.iteration_thread.started.connect(self.iteration_worker.run)
                        self.iteration_worker.finished.connect(self.iteration_thread.quit)
                        self.iteration_worker.finished.connect(self.iteration_worker.deleteLater)
                        # self.iteration_thread.finished.connect(self.iteration_thread.deleteLater)
                        self.iteration_thread.finished.connect(self.iteration_thread_finished)
                    ...
                    if (time.time() - os.path.getmtime(self.queue[0][0]) > 1):
                        ...
                        print(filename)
                        self.cache_worker = CacheCreator(self.cache,self.directory,filename,self.imgctrl,self.flatfield,self.imgmask,self.blkSize)
    ...
    def start_processing(self):
        ...
        if self.process_existing_images_checkbox.isChecked():
            ...
            for filename in existing_files:
                ...
                self.queue.append([filename,results.group("name"),results.group("number"),results.group("ext")])
                print(filename,results.group("name"),results.group("number"),results.group("ext"))
    ...
    def closeEvent(self,evt):
        ...
        if not self.stop_button.isEnabled():
            ...
            evt.accept()
        elif (QtWidgets.QMessageBox.question(self,"Exit","Are you sure you want to stop running over all data?",QtWidgets.QMessageBox.StandardButton.Yes | QtWidgets.QMessageBox.StandardButton.Cancel, QtWidgets.QMessageBox.StandardButton.Cancel) == QtWidgets.QMessageBox.StandardButton.Yes):
            self.clear_queue()
            self.pause()
            evt.accept()
        else:
...
if __name__ == "__main__":
    ...
    if args.imctrl:
        if os.path.exists(PathWrap(args.imctrl)):
            ...
            print("Image control file not found in this directory or in specified directory.")
            imgctrl = None
    else:
        imgctrl = None
