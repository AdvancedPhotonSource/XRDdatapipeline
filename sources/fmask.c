...
static PyObject *fmask_func(PyObject *self, PyObject *args) {
  ...
  */
  ...
  PyArrayObject *outImg = NULL; // optional arg 8
  ...
  double *outImgP; // pointer to (optional) out MAD-mult image array

  /* Parse arguments */
  if (!PyArg_ParseTuple(args, "dOOOOOdd|O", &esdmul,
  ...
  if ((outImg) & (size != PyArray_DIM(outImg, 0))) {
    return makeTypeException("Arg 8 (outImg, optional): array size does not match image.")
  }
  ...
  if (outImg) {
    outImgP = (double *) PyArray_DATA(outImg);
  }
    ...
    */
    ...
    if (outImg) {
      for (int i=0; i < count; ++i) {
        ...
        if (MAD != 0) { // avoid divide by zero
          outImgP[j] = (imageP[j] - median)/MAD;
        }
        else outImgP[j] = 0; // may not be the best value, but this pixel will likely be masked
        if (outImgP[j] > esdmul) {
          outMaskP[j] = 1;
          masked += 1;
        }
