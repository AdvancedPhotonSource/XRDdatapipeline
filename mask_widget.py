...
def choose_file():
    # just tiff for now
    image_file_name = QtWidgets.QFileDialog.getOpenFileName(None,"Choose Image",".","TIFF files (*.tif)")
...
class Polygon(pg.PolyLineROI):
    ...
    def setPoints(self, points, closed=None):
        if len(points) > 0:
            ...
            topRight = QtCore.QPoint(self.image_size[0] + 1,self.image_size[1] + 1) - point0
            # print(bottomLeft,topRight)
            self.maxBounds = QtCore.QRect(bottomLeft,topRight) # only affects first point for some reason
...
class MainWindow(pg.GraphicsLayoutWidget):
    def __init__(self):
        ...
        # Read/Write buttons
        self.load_immask_button = QtWidgets.QPushButton("Load immask") # load in GSASII-compatible immask file
        ...
        self.preview_mask_button = QtWidgets.QPushButton("Preview Mask") # hide UI of all ROIs, precalc and show mask
        self.preview_mask_button.released.connect(self.preview_mask)
        self.save_mask_button = QtWidgets.QPushButton("Save Mask") # require user to preview first?
        ...
        self.polygons_table.setSelectionMode(QtWidgets.QAbstractItemView.SelectionMode.SingleSelection)
    ...
    def preview_mask(self):
        if self.preview_mask_button.text() == "Preview Mask":
            ...
            for i in self.main_image.objects:
                ...
                for it in range(len(corrected_points)):
                    ...
                    if corrected_points[it][0] >= self.main_image.image_data.shape[0]:
                        corrected_points[it] = (self.main_image.image_data.shape[0] - 1, corrected_points[it][1])
                    if corrected_points[it][1] >= self.main_image.image_data.shape[1]:
                        corrected_points[it] = (corrected_points[it][0], self.main_image.image_data.shape[1] - 1)
                    print(corrected_points[it])
                print("Corrected points:", corrected_points)
                i.setPoints(corrected_points)
                # i.stateChanged()
                array_slice = i.getArraySlice(self.main_image.image_data,self.main_image.image,returnSlice=False)
                print(array_slice)
                print(np.abs(array_slice[0][1][0]-array_slice[0][1][1]),np.abs(array_slice[0][0][0]-array_slice[0][0][1]))
                small_poly_mask = i.renderShapeMask(np.abs(array_slice[0][1][0]-array_slice[0][1][1]),np.abs(array_slice[0][0][0]-array_slice[0][0][1]))
                ...
                array_slice = i.getArraySlice(self.main_image.image_data,self.main_image.image)
                print(array_slice)
            ...
            # intensity thresholds
            below_mins = np.nonzero(self.main_image.image_data < self.min_intensity_threshold.value())
            above_maxs = np.nonzero(self.main_image.image_data > self.max_intensity_threshold.value())
            ...
            self.poly_mask_rgba = np.ones((self.predef_mask.shape[0],self.predef_mask.shape[1],4),dtype=np.uint8)*255
    ...
    def save_immask(self):
        ...
        with open(outfilename,'w') as outfile:
            ...
            outfile.write("Arcs:[]\n")
            print(points_list)
            outfile.write("Polygons:{polys}\n".format(polys=points_list).replace('(','[').replace(')',']'))
            ...
            outfile.write("Thresholds:[({image_min}, {image_max}), [{image_min}, {image_max}]]".format(image_min=self.min_intensity_threshold.value(),image_max=self.max_intensity_threshold.value()))
        
    def load_immask(self):
        print("loading")
        infilename = QtWidgets.QFileDialog.getOpenFileName(None,"Choose Image Mask",".","Immask files (*.immask)")[0]
    ...
    def delete_selected_polygon(self):
        selected = self.polygons_table.selectedItems()[0] # list of singular item, as selection mode is single selection
...
class MainImage(pg.GraphicsLayoutWidget):
    def __init__(self):
        ...
        self.predef_mask_data = np.zeros((self.image_data.shape[0],self.image_data.shape[1],4),dtype=np.uint8)
    ...
    def add_polygon(self):
        ...
        polygon = Polygon(positions=[],closed=True,translateSnap=True,rotatable=False,image_size=self.image_data.shape,parent=self.view)
